...
export interface QuestRoute {
  ...
  nodes: QuestNode[]
  operatorBias: number
  params: QuestRouteParams
  title: string
}

export interface QuestRouteParams {
  b: number
  d: number
  s: string
}

export const QUESTATLAS_ORIGIN = 'https://questatlas093.colmena.dev'
...
export function buildQuestRoute(seed: string, difficulty: number, operatorBias: number): QuestRoute {
  const params = createQuestRouteParams(seed, difficulty, operatorBias)
  return buildQuestRouteFromParams(params)
}

export function buildQuestRouteFromParams(params: QuestRouteParams): QuestRoute {
  const difficulty = clamp(Math.round(params.d), 1, 10)
  const operatorBias = clamp(Math.round(params.b), 0, 100)
  const seed = params.s.trim() || '0'
  ...
  return {
    ...
    difficulty,
    entry,
    generatedAt: createGeneratedAt(routeSeed),
    hazards,
    id: `QA-${routeSeed.slice(0, 8)}`,
    loadout,
    nodes,
    operatorBias,
    params: { b: operatorBias, d: difficulty, s: seed },
    title: `${entry} to ${boss}`,
  }
}

export function createMetadataUri(route: QuestRoute, owner: string) {
  return `${QUESTATLAS_ORIGIN}/api/questatlas/metadata?${createRouteQueryString(route.params, owner)}`
}

export function createQuestAtlasMetadata(route: QuestRoute, owner: string): QuestAtlasMetadata {
  const image = createRouteImageUrl(route)
  const externalUrl = `${QUESTATLAS_ORIGIN}/atlas?${createRouteQueryString(route.params)}`
...
export function createQuestAtlasSvg(route: QuestRoute) {
  ...
  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><rect width="100" height="100" fill="#07100f"/><path d="M4 80 C25 60 25 25 48 36 S74 55 96 18" fill="none" stroke="#14443f" stroke-width="16" opacity=".8"/><g>${lines}${points}</g><text x="6" y="92" font-size="6" fill="#f5c86b" font-family="monospace">${route.id}</text></svg>`
}

export function createQuestRouteParams(seed: string, difficulty: number, operatorBias: number): QuestRouteParams {
  return {
    b: clamp(Math.round(operatorBias), 0, 100),
    d: clamp(Math.round(difficulty), 1, 10),
    s: Math.abs(hashNumber(seed.trim() || 'run-093')).toString(36),
  }
}

export function createRouteImageUrl(route: QuestRoute) {
  return `${QUESTATLAS_ORIGIN}/api/questatlas/image?${createRouteQueryString(route.params)}`
}

export function createRouteQueryString(params: QuestRouteParams, owner?: string) {
  const query = new URLSearchParams({
    b: params.b.toString(10),
    d: params.d.toString(10),
    s: params.s,
  })
...
export function decodeMetadataUri(uri: string): null | QuestAtlasMetadata {
  if (uri.startsWith(`${QUESTATLAS_ORIGIN}/api/questatlas/metadata?`)) {
    return null
  }
...
export function parseQuestRouteParams(searchParams: URLSearchParams): null | QuestRouteParams {
  const seed = searchParams.get('s')?.trim()
  const difficulty = Number(searchParams.get('d'))
  const operatorBias = Number(searchParams.get('b'))

  if (!seed || !Number.isFinite(difficulty) || !Number.isFinite(operatorBias)) {
    return null
  }
...
function createGeneratedAt(routeSeed: string) {
  const offsetMinutes = Math.abs(hashNumber(routeSeed)) % 525_600
